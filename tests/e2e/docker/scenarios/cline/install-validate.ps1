# Cline CLI-config validation scenario for task 27.6 (Windows mirror).
#
...
# Fake-Home rooted at a ``Join-Path $env:TEMP``-derived directory so the
# developer's real VSCode user dir is never touched.
#
...
# report fragment shape) follows the cursor scenario from task 27.5.
#
# Path resolution: Cline reads its MCP servers from
# ``<vscode-user>/User/globalStorage/saoudrizwan.claude-dev/settings/
# cline_mcp_settings.json``. The connector honours the
# ``CROSSMEM_VSCODE_USER_DIR`` env override (see
# ``src/crossmem/connectors/_vscode.py``) which we use to redirect the
# lookup into the Fake-Home, sidestepping any real
# ``%APPDATA%/Code/User/...`` that might exist on the runner.

$ErrorActionPreference = 'Stop'

$ScenarioName = 'scenarios/cline/install-validate.ps1'
...
function Get-BackupCount {
    ...
    # mirrors ``ls -1 cline_mcp_settings.json.bak.*`` in the bash
    # script. ``Measure-Object`` exposes ``.Count`` regardless of how
    # many items match (zero, one, many).
    @(Get-ChildItem -LiteralPath $Directory -Filter 'cline_mcp_settings.json.bak.*' -ErrorAction SilentlyContinue) `
        | Measure-Object `
...
try {
    ...
    $FakeHome = New-Item -ItemType Directory `
        -Path (Join-Path $env:TEMP ("crossmem-e2e-cline-" + [Guid]::NewGuid().Guid))
    ...
    # config under our Fake-Home regardless of host platform. The
    # override is documented in ``src/crossmem/connectors/_vscode.py``
    # and is the same hook the unit tests use.
    $VscodeUserDir = Join-Path $FakeHome.FullName 'Code'
    $env:CROSSMEM_VSCODE_USER_DIR = $VscodeUserDir

    $ConfigDir = Join-Path $VscodeUserDir `
