...
#
# Why a dedicated fake HOME — the host's real ``~/.claude.json`` (if
# any) would otherwise be backed up + rewritten by the scenario; the
# container layer adds a second safety net, but redirecting ``HOME``
# inside the script means the same script is safe to run on the host
...
SCENARIO_NAME="scenarios/claude-code/install-validate.sh"
LOG_PATH="${LOG_PATH:-reports/claude-code/install-validate.log}"
...
# Seed a minimal valid Claude-Code config: one unrelated top-level key
...
backup_count=$(find "${FAKE_HOME}" -maxdepth 1 -name '.claude.json.bak.*' \
    -type f | wc -l)
if [[ "${backup_count}" -lt 1 ]]; then
    fail "expected at least one .claude.json.bak.* after first install"
fi
...
backup_count_after_second=$(find "${FAKE_HOME}" -maxdepth 1 \
    -name '.claude.json.bak.*' -type f | wc -l)
...
if [[ "${STATUS}" == "pass" ]]; then
    echo "==> claude-code install-validate: OK"
    # ``emit_report`` runs via EXIT trap.
    exit 0
fi

echo "==> claude-code install-validate: FAIL (${FAIL_REASON})" >&2
exit 1
