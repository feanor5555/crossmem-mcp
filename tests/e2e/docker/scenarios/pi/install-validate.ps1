# pi install-validate scenario, PowerShell mirror (task 27.8).
#
# Mirrors ``install-validate.sh`` in lockstep so the Windows runner
# (task 27.2) gets the same install -> validate -> idempotency ->
# uninstall coverage for the Pi connector. Pi has no native MCP
# support, but the ``pi-mcp-adapter`` npm package reads MCP server
# configs from ``~/.pi/agent/mcp.json`` (HOME-relative on every
# platform). The scenario seeds an isolated fake home with that nested
# directory and a minimal valid Pi config (one unrelated top-level
# entry plus an existing ``mcpServers`` slot), drives the full
...
$ScenarioName = 'scenarios/pi/install-validate.ps1'
$LogPath = if ($env:LOG_PATH) { $env:LOG_PATH } else { 'reports/pi/install-validate.log' }
...
$FakeHome = Join-Path ([System.IO.Path]::GetTempPath()) ("crossmem-e2e-pi-" + [Guid]::NewGuid().ToString('N'))
...
# crossmem installer resolves ``~/.pi/agent/mcp.json`` to the fake dir.
$env:HOME = $FakeHome
...
$ConfigDir = Join-Path $FakeHome '.pi' | Join-Path -ChildPath 'agent'
...
if ($script:Status -eq 'pass') {
    Write-Host '==> pi install-validate: OK'
    exit 0
}

Write-Error -ErrorAction Continue -Message "==> pi install-validate: FAIL ($($script:FailReason))"
exit 1
