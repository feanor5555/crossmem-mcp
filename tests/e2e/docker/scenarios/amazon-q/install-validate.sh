...
# ``~/.aws/amazonq/mcp.json`` (if any) would otherwise be backed up +
...
#
# Why ``~/.aws/amazonq/mcp.json`` — the Amazon Q CLI stores its MCP
# configuration in a per-user ``mcp.json`` under ``~/.aws/amazonq`` on
# every platform (see ``_amazonq_path`` in
...
SCENARIO_NAME="scenarios/amazon-q/install-validate.sh"
LOG_PATH="${LOG_PATH:-reports/amazon-q/install-validate.log}"
...
# Pre-create the ``~/.aws/amazonq`` parent directory so the seed write
# succeeds; ``crossmem install`` would create it itself, but we want
# a deterministic on-disk shape before the first invocation.
CONFIG_DIR="${FAKE_HOME}/.aws/amazonq"
CONFIG_PATH="${CONFIG_DIR}/mcp.json"
mkdir -p "${CONFIG_DIR}"

# Seed a minimal valid Amazon Q CLI config: one unrelated top-level key
...
if [[ "${STATUS}" == "pass" ]]; then
    echo "==> amazon-q install-validate: OK"
    # ``emit_report`` runs via EXIT trap.
    exit 0
fi

echo "==> amazon-q install-validate: FAIL (${FAIL_REASON})" >&2
exit 1
