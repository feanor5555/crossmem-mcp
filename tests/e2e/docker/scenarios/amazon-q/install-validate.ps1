# amazon-q install-validate scenario, PowerShell mirror (task 27.14).
#
...
# minimal valid Amazon Q CLI config (one unrelated top-level entry
# plus an existing ``mcpServers`` slot), drives the full
# ``crossmem install`` -> ``crossmem install`` -> ``crossmem uninstall``
# lifecycle, and emits a single JSON report fragment to
# ``$env:REPORT_PATH`` in the schema fixed by task 27.1.
#
...
#
# Why ``~/.aws/amazonq/mcp.json`` on every platform — the Amazon Q CLI
# connector resolves its config via ``Path.home() / ".aws" / "amazonq"
# / "mcp.json"`` unconditionally (see ``_amazonq_path`` in
# ``src/crossmem/connectors/registry.py``); there is no Windows-
# specific branch (unlike OpenCode's ``%APPDATA%`` redirect).
# Remapping ``HOME`` + ``USERPROFILE`` is therefore sufficient.
...
$ScenarioName = 'scenarios/amazon-q/install-validate.ps1'
$LogPath = if ($env:LOG_PATH) { $env:LOG_PATH } else { 'reports/amazon-q/install-validate.log' }
...
$FakeHome = Join-Path ([System.IO.Path]::GetTempPath()) ("crossmem-e2e-amazonq-" + [Guid]::NewGuid().ToString('N'))
...
# crossmem installer resolves ``~/.aws/amazonq/mcp.json`` to the fake
# dir.
$env:HOME = $FakeHome
$env:USERPROFILE = $FakeHome

$ConfigDir = Join-Path (Join-Path $FakeHome '.aws') 'amazonq'
...
try {
    ...
    # Validate the rewritten config against the Amazon Q CLI MCP layout.
    ...
    $backups = Get-ChildItem -LiteralPath $ConfigDir -Filter 'mcp.json.bak.*' `
        -File -ErrorAction SilentlyContinue
    if (($backups | Measure-Object).Count -lt 1) {
        Set-Failure 'expected at least one mcp.json.bak.* after first install'
    }
    ...
    $backupsAfter = Get-ChildItem -LiteralPath $ConfigDir -Filter 'mcp.json.bak.*' `
        -File -ErrorAction SilentlyContinue
...
if ($script:Status -eq 'pass') {
    Write-Host '==> amazon-q install-validate: OK'
    exit 0
}

Write-Error -ErrorAction Continue -Message "==> amazon-q install-validate: FAIL ($($script:FailReason))"
exit 1
