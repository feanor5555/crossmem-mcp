#!/usr/bin/env bash
# opencode install-validate scenario (task 27.7).
#
# Runs inside the Linux runner image (``Dockerfile.linux``) where
# ``crossmem`` is already on ``PATH`` via pipx. The scenario seeds an
# isolated fake home with a minimal valid OpenCode config (one unrelated
# top-level entry plus an existing ``mcp`` slot), drives the full
# ``crossmem install`` -> ``crossmem install`` -> ``crossmem uninstall``
# lifecycle, and emits a single JSON report fragment to ``$REPORT_PATH``
# in the schema fixed by task 27.1.
#
# Why a dedicated fake HOME — the host's real
# ``~/.config/opencode/opencode.json`` (if any) would otherwise be backed
# up + rewritten by the scenario; the container layer adds a second
# safety net, but redirecting ``HOME`` inside the script means the same
# script is safe to run on the host for ad-hoc debugging.
#
# Why two ``crossmem install`` invocations — the spec asks for an
# idempotency proof: the second run must not duplicate the ``crossmem``
# MCP entry inside the config file. We also assert that the on-disk
# backup count stays bounded by ``BACKUP_RETENTION`` so a regression
# that disables the prune step trips the scenario.
#
...
SCENARIO_NAME="scenarios/opencode/install-validate.sh"
LOG_PATH="${LOG_PATH:-reports/opencode/install-validate.log}"
...
# OpenCode on Linux/macOS resolves the config via ``Path.home()`` to
# ``~/.config/opencode/opencode.json``. Pre-create the parent directory
# so the seed write succeeds; ``crossmem install`` would create it
# itself but we want a deterministic on-disk shape before the first
# invocation.
CONFIG_DIR="${FAKE_HOME}/.config/opencode"
CONFIG_PATH="${CONFIG_DIR}/opencode.json"
mkdir -p "${CONFIG_DIR}"

# Seed a minimal valid OpenCode config: one unrelated top-level key
# plus a pre-existing ``mcp`` entry that must survive uninstall. JSON
...
assert "mcp" in data, "missing top-level mcp"
servers = data["mcp"]
...
backup_count=$(find "${CONFIG_DIR}" -maxdepth 1 -name 'opencode.json.bak.*' \
    -type f | wc -l)
if [[ "${backup_count}" -lt 1 ]]; then
    fail "expected at least one opencode.json.bak.* after first install"
fi
...
backup_count_after_second=$(find "${CONFIG_DIR}" -maxdepth 1 \
    -name 'opencode.json.bak.*' -type f | wc -l)
...
if [[ "${STATUS}" == "pass" ]]; then
    echo "==> opencode install-validate: OK"
    # ``emit_report`` runs via EXIT trap.
    exit 0
fi

echo "==> opencode install-validate: FAIL (${FAIL_REASON})" >&2
exit 1
