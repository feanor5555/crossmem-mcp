...
# ``~/.gemini/settings.json`` (if any) would otherwise be backed up +
# rewritten by the scenario; the container layer adds a second safety
# net, but redirecting ``HOME`` inside the script means the same
...
#
# Why ``~/.gemini/settings.json`` — Gemini CLI stores its user-level
# configuration in a per-user ``settings.json`` under ``~/.gemini`` on
# every platform (see ``_gemini_path`` in
# ``src/crossmem/connectors/registry.py``). MCP servers live under the
# default top-level ``mcpServers`` key.
...
SCENARIO_NAME="scenarios/gemini-cli/install-validate.sh"
LOG_PATH="${LOG_PATH:-reports/gemini-cli/install-validate.log}"
...
# Pre-create the ``~/.gemini`` parent directory so the seed write
# succeeds; ``crossmem install`` would create it itself, but we want
# a deterministic on-disk shape before the first invocation.
CONFIG_DIR="${FAKE_HOME}/.gemini"
CONFIG_PATH="${CONFIG_DIR}/settings.json"
mkdir -p "${CONFIG_DIR}"

# Seed a minimal valid Gemini CLI config: one unrelated top-level key
...
# Validate the rewritten config against the Gemini CLI MCP layout.
...
if [[ "${STATUS}" == "pass" ]]; then
    echo "==> gemini-cli install-validate: OK"
    # ``emit_report`` runs via EXIT trap.
    exit 0
fi

echo "==> gemini-cli install-validate: FAIL (${FAIL_REASON})" >&2
exit 1
