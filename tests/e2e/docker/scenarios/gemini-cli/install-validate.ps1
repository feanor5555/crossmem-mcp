# gemini-cli install-validate scenario, PowerShell mirror (task 27.11).
#
...
# minimal valid Gemini CLI config (one unrelated top-level entry plus
...
#
# Why ``~/.gemini/settings.json`` on every platform — the Gemini CLI
# connector resolves its config via ``Path.home() / ".gemini" /
# "settings.json"`` unconditionally (see ``_gemini_path`` in
# ``src/crossmem/connectors/registry.py``); there is no Windows-
# specific branch (unlike OpenCode's ``%APPDATA%`` redirect). Remapping
# ``HOME`` + ``USERPROFILE`` is therefore sufficient.
...
$ScenarioName = 'scenarios/gemini-cli/install-validate.ps1'
$LogPath = if ($env:LOG_PATH) { $env:LOG_PATH } else { 'reports/gemini-cli/install-validate.log' }
...
$FakeHome = Join-Path ([System.IO.Path]::GetTempPath()) ("crossmem-e2e-gemini-" + [Guid]::NewGuid().ToString('N'))
...
# crossmem installer resolves ``~/.gemini/settings.json`` to the fake
# dir.
$env:HOME = $FakeHome
$env:USERPROFILE = $FakeHome

$ConfigDir = Join-Path $FakeHome '.gemini'
...
if ($script:Status -eq 'pass') {
    Write-Host '==> gemini-cli install-validate: OK'
    exit 0
}

Write-Error -ErrorAction Continue -Message "==> gemini-cli install-validate: FAIL ($($script:FailReason))"
exit 1
