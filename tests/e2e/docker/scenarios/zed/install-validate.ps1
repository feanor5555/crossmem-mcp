# Zed install-validate scenario, PowerShell mirror (task 27.15).
#
...
# minimal valid Zed config (one unrelated top-level entry plus an
# existing ``context_servers`` slot), drives the full ``crossmem
# install`` -> ``crossmem install`` -> ``crossmem uninstall`` lifecycle,
# and emits a single JSON report fragment to ``$env:REPORT_PATH`` in
# the schema fixed by task 27.1.
#
...
#
# Why ``%APPDATA%/Zed/settings.json`` — Zed reads its user settings
# from ``%APPDATA%/Zed/settings.json`` on Windows (see ``_zed_path``
# in ``src/crossmem/connectors/registry.py``); on Linux/macOS the
# bash sibling targets ``~/.config/zed/settings.json``. Both scripts
# write to the same ``context_servers`` MCP slot.
...
$ScenarioName = 'scenarios/zed/install-validate.ps1'
$LogPath = if ($env:LOG_PATH) { $env:LOG_PATH } else { 'reports/zed/install-validate.log' }
...
$FakeHome = Join-Path ([System.IO.Path]::GetTempPath()) ("crossmem-e2e-zed-" + [Guid]::NewGuid().ToString('N'))
...
# crossmem installer's home-derived paths resolve to the fake dir.
# ``APPDATA`` is also redirected: Zed reads its settings from
...
$ConfigDir = Join-Path $FakeAppData 'Zed'
...
try {
    ...
    # Validate the rewritten config against the Zed MCP layout.
    $data = Get-Content -LiteralPath $ConfigPath -Raw | ConvertFrom-Json
    if (-not ($data.PSObject.Properties.Name -contains 'context_servers')) {
        Set-Failure 'missing top-level context_servers'
    }
    elseif (-not ($data.context_servers.PSObject.Properties.Name -contains 'crossmem')) {
        Set-Failure 'crossmem entry not registered'
    }
    else {
        $entry = $data.context_servers.crossmem
        foreach ($field in @('command', 'args', 'env')) {
            if (-not ($entry.PSObject.Properties.Name -contains $field)) {
                Set-Failure "crossmem entry missing '$field'"
            }
        }
        if (-not ($data.context_servers.PSObject.Properties.Name -contains 'other')) {
    ...
    $backups = Get-ChildItem -LiteralPath $ConfigDir -Filter 'settings.json.bak.*' `
        -File -ErrorAction SilentlyContinue
    if (($backups | Measure-Object).Count -lt 1) {
        Set-Failure 'expected at least one settings.json.bak.* after first install'
    }
    ...
    $data = Get-Content -LiteralPath $ConfigPath -Raw | ConvertFrom-Json
    $crossmemKeys = @($data.context_servers.PSObject.Properties.Name | Where-Object { $_ -eq 'crossmem' })
    ...
    $backupsAfter = Get-ChildItem -LiteralPath $ConfigDir -Filter 'settings.json.bak.*' `
        -File -ErrorAction SilentlyContinue
    ...
    $data = Get-Content -LiteralPath $ConfigPath -Raw | ConvertFrom-Json
    if ($data.context_servers.PSObject.Properties.Name -contains 'crossmem') {
...
if ($script:Status -eq 'pass') {
    Write-Host '==> zed install-validate: OK'
    exit 0
}

Write-Error -ErrorAction Continue -Message "==> zed install-validate: FAIL ($($script:FailReason))"
exit 1
