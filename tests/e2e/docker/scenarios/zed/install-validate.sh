#!/usr/bin/env bash
# Zed install-validate scenario (task 27.15).
#
# Runs inside the Linux runner image (``Dockerfile.linux``) where
# ``crossmem`` is already on ``PATH`` via pipx. The scenario seeds an
# isolated fake home with a minimal valid Zed config (one unrelated
# top-level entry plus an existing ``context_servers`` slot), drives
# the full ``crossmem install`` -> ``crossmem install`` ->
...
# ``name="zed"`` in ``src/crossmem/connectors/registry.py``). Hardcoding
# the key here is intentional: a regression that switched the Zed
# connector to a different key would silently break Zed itself, and
# this scenario is the cheapest place to catch it.
#
# Why a dedicated fake HOME — the host's real ``~/.config/zed/
# settings.json`` (if any) would otherwise be backed up + rewritten by
# the scenario; the container layer adds a second safety net, but
# redirecting ``HOME`` inside the script means the same script is safe
# to run on the host for ad-hoc debugging.
#
...
SCENARIO_NAME="scenarios/zed/install-validate.sh"
LOG_PATH="${LOG_PATH:-reports/zed/install-validate.log}"
...
# Zed config lives at ``~/.config/zed/settings.json`` on Linux/macOS
# (mirrors ``_zed_path`` in ``src/crossmem/connectors/registry.py``).
CONFIG_DIR="${FAKE_HOME}/.config/zed"
CONFIG_PATH="${CONFIG_DIR}/settings.json"
mkdir -p "${CONFIG_DIR}"

# Seed a minimal valid Zed config: one unrelated top-level key plus a
# pre-existing ``context_servers`` entry that must survive uninstall.
...
assert "context_servers" in data, "missing top-level context_servers"
...
backup_count=$(find "${CONFIG_DIR}" -maxdepth 1 -name 'settings.json.bak.*' \
    -type f | wc -l)
if [[ "${backup_count}" -lt 1 ]]; then
    fail "expected at least one settings.json.bak.* after first install"
fi
...
backup_count_after_second=$(find "${CONFIG_DIR}" -maxdepth 1 \
    -name 'settings.json.bak.*' -type f | wc -l)
...
if [[ "${STATUS}" == "pass" ]]; then
    echo "==> zed install-validate: OK"
    # ``emit_report`` runs via EXIT trap.
    exit 0
fi

echo "==> zed install-validate: FAIL (${FAIL_REASON})" >&2
exit 1
