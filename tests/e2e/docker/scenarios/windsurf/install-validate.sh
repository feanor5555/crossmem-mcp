#!/usr/bin/env bash
# windsurf install-validate scenario (task 27.13).
#
# Runs inside the Linux runner image (``Dockerfile.linux``) where
# ``crossmem`` is already on ``PATH`` via pipx. The scenario seeds an
# isolated fake home with a minimal valid Windsurf MCP config (one
# unrelated top-level entry plus an existing ``mcpServers`` slot),
# drives the full ``crossmem install`` -> ``crossmem install`` ->
# ``crossmem uninstall`` lifecycle, and emits a single JSON report
# fragment to ``$REPORT_PATH`` in the schema fixed by task 27.1.
#
# Why a dedicated fake HOME — the host's real
# ``~/.codeium/windsurf/mcp_config.json`` (if any) would otherwise be
# backed up + rewritten by the scenario; the container layer adds a
# second safety net, but redirecting ``HOME`` inside the script means
# the same script is safe to run on the host for ad-hoc debugging.
#
...
#
# Why ``~/.codeium/windsurf/mcp_config.json`` — Windsurf (Codeium)
# stores its user-level MCP configuration in a per-user
# ``mcp_config.json`` under ``~/.codeium/windsurf`` on every platform
# (see ``_windsurf_path`` in ``src/crossmem/connectors/registry.py``).
# MCP servers live under the default top-level ``mcpServers`` key.
...
SCENARIO_NAME="scenarios/windsurf/install-validate.sh"
LOG_PATH="${LOG_PATH:-reports/windsurf/install-validate.log}"
...
# Pre-create the ``~/.codeium/windsurf`` parent directory so the seed
# write succeeds; ``crossmem install`` would create it itself, but we
# want a deterministic on-disk shape before the first invocation.
CONFIG_DIR="${FAKE_HOME}/.codeium/windsurf"
CONFIG_PATH="${CONFIG_DIR}/mcp_config.json"
mkdir -p "${CONFIG_DIR}"

# Seed a minimal valid Windsurf MCP config: one unrelated top-level key
# plus a pre-existing ``mcpServers`` entry that must survive uninstall.
# JSON is hand-written (not generated) so the assertions further down
# can match the literal field names spec-style.
...
# Validate the rewritten config against the Windsurf MCP layout.
...
backup_count=$(find "${CONFIG_DIR}" -maxdepth 1 -name 'mcp_config.json.bak.*' \
    -type f | wc -l)
if [[ "${backup_count}" -lt 1 ]]; then
    fail "expected at least one mcp_config.json.bak.* after first install"
fi
...
backup_count_after_second=$(find "${CONFIG_DIR}" -maxdepth 1 \
    -name 'mcp_config.json.bak.*' -type f | wc -l)
...
if [[ "${STATUS}" == "pass" ]]; then
    echo "==> windsurf install-validate: OK"
    # ``emit_report`` runs via EXIT trap.
    exit 0
fi

echo "==> windsurf install-validate: FAIL (${FAIL_REASON})" >&2
exit 1
