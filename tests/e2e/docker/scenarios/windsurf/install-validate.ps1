# windsurf install-validate scenario, PowerShell mirror (task 27.13).
#
...
# minimal valid Windsurf MCP config (one unrelated top-level entry plus
...
#
# Why ``~/.codeium/windsurf/mcp_config.json`` on every platform — the
# Windsurf connector resolves its config via ``Path.home() / ".codeium"
# / "windsurf" / "mcp_config.json"`` unconditionally (see
# ``_windsurf_path`` in ``src/crossmem/connectors/registry.py``); there
# is no Windows-specific branch (unlike OpenCode's ``%APPDATA%``
# redirect). Remapping ``HOME`` + ``USERPROFILE`` is therefore
# sufficient.
...
$ScenarioName = 'scenarios/windsurf/install-validate.ps1'
$LogPath = if ($env:LOG_PATH) { $env:LOG_PATH } else { 'reports/windsurf/install-validate.log' }
...
$FakeHome = Join-Path ([System.IO.Path]::GetTempPath()) ("crossmem-e2e-windsurf-" + [Guid]::NewGuid().ToString('N'))
...
# crossmem installer resolves ``~/.codeium/windsurf/mcp_config.json``
# to the fake dir.
$env:HOME = $FakeHome
$env:USERPROFILE = $FakeHome

$ConfigDir = Join-Path (Join-Path $FakeHome '.codeium') 'windsurf'
...
try {
    ...
    $backups = Get-ChildItem -LiteralPath $ConfigDir -Filter 'mcp_config.json.bak.*' `
        -File -ErrorAction SilentlyContinue
    if (($backups | Measure-Object).Count -lt 1) {
        Set-Failure 'expected at least one mcp_config.json.bak.* after first install'
    }
    ...
    $backupsAfter = Get-ChildItem -LiteralPath $ConfigDir -Filter 'mcp_config.json.bak.*' `
        -File -ErrorAction SilentlyContinue
...
if ($script:Status -eq 'pass') {
    Write-Host '==> windsurf install-validate: OK'
    exit 0
}

Write-Error -ErrorAction Continue -Message "==> windsurf install-validate: FAIL ($($script:FailReason))"
exit 1
