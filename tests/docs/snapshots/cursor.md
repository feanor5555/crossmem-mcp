...
config_path_linux: ~/.cursor/mcp.json
config_path_mac: ~/.cursor/mcp.json
config_path_win: '%USERPROFILE%\.cursor\mcp.json'
restart_hint: Fully quit and relaunch cursor so it re-reads the MCP config and re-spawns the crossmem server.
---
# Install crossmem for cursor

This file was generated by `crossmem docs install --cli cursor` from the
...
- The target CLI (cursor) is already installed and starts cleanly.
...
Locate the cursor MCP config file for the host OS:
...
1. Fully quit and relaunch cursor so it re-reads the MCP config and re-spawns the crossmem server.
2. From inside cursor, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in cursor** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that cursor was fully
  restarted.
