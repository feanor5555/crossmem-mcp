...
config_path_linux: ~/.config/opencode/opencode.json
config_path_mac: ~/.config/opencode/opencode.json
config_path_win: '%APPDATA%\opencode\opencode.json'
restart_hint: The new config is picked up on the next opencode invocation; no manual restart is required.
---
# Install crossmem for opencode

This file was generated by `crossmem docs install --cli opencode` from the
...
- The target CLI (opencode) is already installed and starts cleanly.
...
Locate the opencode MCP config file for the host OS:
...
1. The new config is picked up on the next opencode invocation; no manual restart is required.
2. From inside opencode, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in opencode** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that opencode was fully
  restarted.
