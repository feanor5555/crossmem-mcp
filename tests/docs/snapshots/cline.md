...
config_path_linux: ~/.config/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json
config_path_mac: ~/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json
config_path_win: '%APPDATA%\Code\User\globalStorage\saoudrizwan.claude-dev\settings\cline_mcp_settings.json'
restart_hint: Fully quit and relaunch cline so it re-reads the MCP config and re-spawns the crossmem server.
---
# Install crossmem for cline

This file was generated by `crossmem docs install --cli cline` from the
...
- The target CLI (cline) is already installed and starts cleanly.
...
Locate the cline MCP config file for the host OS:
...
1. Fully quit and relaunch cline so it re-reads the MCP config and re-spawns the crossmem server.
2. From inside cline, ask the model to call the `query` MCP tool with a
...
- `install_doc_gemini`
- `backup_retention_claude_code`
- `backup_retention_gemini`
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in cline** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that cline was fully
  restarted.
