...
config_path_linux: ~/.codeium/windsurf/mcp_config.json
config_path_mac: ~/.codeium/windsurf/mcp_config.json
config_path_win: '%USERPROFILE%\.codeium\windsurf\mcp_config.json'
restart_hint: Fully quit and relaunch windsurf so it re-reads the MCP config and re-spawns the crossmem server.
---
# Install crossmem for windsurf

This file was generated by `crossmem docs install --cli windsurf` from the
...
- The target CLI (windsurf) is already installed and starts cleanly.
...
Locate the windsurf MCP config file for the host OS:
...
1. Fully quit and relaunch windsurf so it re-reads the MCP config and re-spawns the crossmem server.
2. From inside windsurf, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in windsurf** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that windsurf was fully
  restarted.
