...
config_path_linux: ~/.config/zed/settings.json
config_path_mac: ~/.config/zed/settings.json
config_path_win: '%APPDATA%\Zed\settings.json'
restart_hint: Fully quit and relaunch zed so it re-reads the MCP config and re-spawns the crossmem server.
---
# Install crossmem for zed

This file was generated by `crossmem docs install --cli zed` from the
...
- The target CLI (zed) is already installed and starts cleanly.
...
Locate the zed MCP config file for the host OS:
...
1. Fully quit and relaunch zed so it re-reads the MCP config and re-spawns the crossmem server.
2. From inside zed, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in zed** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that zed was fully
  restarted.
