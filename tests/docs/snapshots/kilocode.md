...
config_path_linux: ~/.config/Code/User/globalStorage/kilocode.kilo-code/settings/mcp_settings.json
config_path_mac: ~/Library/Application Support/Code/User/globalStorage/kilocode.kilo-code/settings/mcp_settings.json
config_path_win: '%APPDATA%\Code\User\globalStorage\kilocode.kilo-code\settings\mcp_settings.json'
restart_hint: Fully quit and relaunch kilocode so it re-reads the MCP config and re-spawns the crossmem server.
---
# Install crossmem for kilocode

This file was generated by `crossmem docs install --cli kilocode` from the
...
- The target CLI (kilocode) is already installed and starts cleanly.
...
Locate the kilocode MCP config file for the host OS:
...
1. Fully quit and relaunch kilocode so it re-reads the MCP config and re-spawns the crossmem server.
2. From inside kilocode, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in kilocode** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that kilocode was fully
  restarted.
