...
config_path_linux: ~/.aws/amazonq/mcp.json
config_path_mac: ~/.aws/amazonq/mcp.json
config_path_win: '%USERPROFILE%\.aws\amazonq\mcp.json'
restart_hint: The new config is picked up on the next amazonq invocation; no manual restart is required.
---
# Install crossmem for amazonq

This file was generated by `crossmem docs install --cli amazonq` from the
...
- The target CLI (amazonq) is already installed and starts cleanly.
...
Locate the amazonq MCP config file for the host OS:
...
1. The new config is picked up on the next amazonq invocation; no manual restart is required.
2. From inside amazonq, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in amazonq** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that amazonq was fully
  restarted.
