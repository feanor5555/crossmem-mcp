...
config_path_linux: ~/.claude.json
config_path_mac: ~/.claude.json
config_path_win: '%USERPROFILE%\.claude.json'
restart_hint: The new config is picked up on the next claude_code invocation; no manual restart is required.
---
# Install crossmem for claude_code

This file was generated by `crossmem docs install --cli claude_code` from the
...
- The target CLI (claude_code) is already installed and starts cleanly.
...
Locate the claude_code MCP config file for the host OS:
...
1. The new config is picked up on the next claude_code invocation; no manual restart is required.
2. From inside claude_code, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in claude_code** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that claude_code was fully
  restarted.
