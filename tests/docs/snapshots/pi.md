...
config_path_linux: ~/.pi/agent/mcp.json
config_path_mac: ~/.pi/agent/mcp.json
config_path_win: '%USERPROFILE%\.pi\agent\mcp.json'
restart_hint: The new config is picked up on the next pi invocation; no manual restart is required.
---
# Install crossmem for pi

This file was generated by `crossmem docs install --cli pi` from the
...
- The target CLI (pi) is already installed and starts cleanly.
...
Locate the pi MCP config file for the host OS:

- Linux: `~/.pi/agent/mcp.json`
- macOS: `~/.pi/agent/mcp.json`
...
1. The new config is picked up on the next pi invocation; no manual restart is required.
2. From inside pi, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in pi** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that pi was fully
  restarted.
