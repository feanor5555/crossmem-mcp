...
config_path_linux: ~/.gemini/settings.json
config_path_mac: ~/.gemini/settings.json
config_path_win: '%USERPROFILE%\.gemini\settings.json'
restart_hint: The new config is picked up on the next gemini invocation; no manual restart is required.
---
# Install crossmem for gemini

This file was generated by `crossmem docs install --cli gemini` from the
...
- The target CLI (gemini) is already installed and starts cleanly.
...
Locate the gemini MCP config file for the host OS:
...
1. The new config is picked up on the next gemini invocation; no manual restart is required.
2. From inside gemini, ask the model to call the `query` MCP tool with a
...
- **`crossmem` not found after `pipx install`** — ensure
  ...
  new shell.
- **MCP server does not appear in gemini** — verify the config file path
  for the host OS, confirm the JSON is valid
  (`python -m json.tool < <path>`), and check that gemini was fully
  restarted.
