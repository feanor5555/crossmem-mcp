"""Static validation for the Windsurf install-validate E2E scenario (task 27.13).
...
requires a Docker daemon plus a freshly built crossmem wheel inside the
container; both are gated behind the manual entry-points
...
turn the manual command into a no-op for Windsurf:
...
* both seed a minimal **valid** Windsurf MCP config (the ``mcpServers``
  shape ``~/.codeium/windsurf/mcp_config.json`` carries) into a
...
* the parity invariants between the two scripts — same path under
  ``~/.codeium/windsurf/mcp_config.json``, same neighbour-entry name
  (proves the "andere Eintraege bleiben unveraendert" clause is
  actually checked on both platforms) — match exactly so a fix on one
  platform is guaranteed to land on the other.
...
SCENARIO_DIR = REPO_ROOT / "tests" / "e2e" / "docker" / "scenarios" / "windsurf"
...
def test_windsurf_scenario_layout_exists() -> None:
    """Both spec-mandated scripts exist at the TODO 27.13 paths.
...
def test_bash_script_seeds_fake_home_with_minimal_windsurf_config() -> None:
    """The bash script must seed a Fake-Home containing a valid Windsurf config.

    Windsurf's MCP config lives at
    ``~/.codeium/windsurf/mcp_config.json`` on every platform (mirrors
    ``_windsurf_path`` in ``src/crossmem/connectors/registry.py``) and
    the server map is under the top-level ``mcpServers`` key. The
    ...
    assert ".codeium" in content and "windsurf" in content, (
        "bash script does not target ~/.codeium/windsurf"
    )
    assert "mcp_config.json" in content, (
        "bash script does not target the mcp_config.json filename"
    )
...
def test_bash_script_validates_command_args_env_keys() -> None:
    ...
    Windsurf expects each ``mcpServers`` entry to carry these three
...
def test_bash_script_checks_bak_present() -> None:
    """After the first install the script must verify ``mcp_config.json.bak.*``.
    ...
    """
    content = _read(BASH_SCRIPT)
    assert re.search(r"mcp_config\.json\.bak", content) or re.search(
        r"\.bak\b", content
...
def test_bash_script_preserves_neighbour_entry_after_uninstall() -> None:
    ...
    """
    content = _read(BASH_SCRIPT)
    # The post-uninstall block must compare against ``servers`` (or
    # ``mcpServers``) with a negated membership test.
...
def test_ps1_script_seeds_fake_home_with_minimal_windsurf_config() -> None:
    ...
    The Windsurf config path is platform-independent
    (``~/.codeium/windsurf/mcp_config.json``), so no ``%APPDATA%``
    redirect is required — unlike OpenCode.
    """
    content = _read(PS1_SCRIPT)
    ...
    assert ".codeium" in content and "windsurf" in content, (
        "PowerShell script does not target ~/.codeium/windsurf"
    )
    assert "mcp_config.json" in content, (
        "PowerShell script does not target the mcp_config.json filename"
    )
    assert "mcpServers" in content, (
...
def test_ps1_script_validates_command_args_env_keys() -> None:
    ...
    Keeps the Linux + Windows runners' coverage symmetric: a Windsurf
...
def test_ps1_script_checks_bak_present() -> None:
    """PS1 script must check the ``mcp_config.json.bak`` glob after first install.
...
def test_scripts_target_the_same_windsurf_path() -> None:
    """Both scripts must point at ``~/.codeium/windsurf/mcp_config.json``.

    Windsurf's MCP config path is platform-independent (see
    ``_windsurf_path`` in ``crossmem.connectors.registry``). The
    Windows script may spell the separator either way (``/`` or
    ``\\``) since PowerShell accepts both — but both must reference
    the same final component ``mcp_config.json`` under a
    ``.codeium/windsurf`` directory.
    """
    bash = _read(BASH_SCRIPT)
    ps1 = _read(PS1_SCRIPT)
    assert ".codeium" in bash and "windsurf" in bash and "mcp_config.json" in bash
    assert ".codeium" in ps1 and "windsurf" in ps1 and "mcp_config.json" in ps1
...
def test_scripts_use_the_same_scenario_name() -> None:
    ...
    bash = _read(BASH_SCRIPT)
    ps1 = _read(PS1_SCRIPT)
    assert "scenarios/windsurf/install-validate.sh" in bash, (
        "bash report fragment does not embed its canonical scenario name"
    )
    assert "scenarios/windsurf/install-validate.ps1" in ps1, (
...
@pytest.mark.parametrize(
    "rel_path",
    [
        "tests/e2e/docker/scenarios/windsurf/install-validate.sh",
    ],
)
