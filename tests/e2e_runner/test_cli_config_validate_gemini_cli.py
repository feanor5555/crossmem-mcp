"""Static validation for the Gemini CLI install-validate E2E scenario (task 27.11).

Like the analog guards for Claude Code
(:mod:`tests.e2e_runner.test_scenario_claude_code`) and Cursor
(:mod:`tests.e2e_runner.test_cli_config_validate_cursor`), this module
asserts the **shape** of the per-CLI scenario artefacts rather than
executing them. Running the bash and PowerShell scripts end-to-end
...
turn the manual command into a no-op for Gemini CLI:
...
* both seed a minimal **valid** Gemini CLI MCP config (the
  ``mcpServers`` shape ``~/.gemini/settings.json`` carries) into a
...
* the parity invariants between the two scripts — same path under
  ``~/.gemini/settings.json``, same neighbour-entry name (proves the
...
SCENARIO_DIR = REPO_ROOT / "tests" / "e2e" / "docker" / "scenarios" / "gemini-cli"
...
def test_gemini_scenario_layout_exists() -> None:
    """Both spec-mandated scripts exist at the TODO 27.11 paths.
...
def test_bash_script_seeds_fake_home_with_minimal_gemini_config() -> None:
    """The bash script must seed a Fake-Home containing a valid Gemini config.

    Gemini CLI's MCP config lives at ``~/.gemini/settings.json`` on
    every platform (mirrors ``_gemini_path`` in
    ...
    assert ".gemini" in content and "settings.json" in content, (
        "bash script does not target ~/.gemini/settings.json"
    )
...
def test_bash_script_validates_command_args_env_keys() -> None:
    ...
    Gemini CLI expects each ``mcpServers`` entry to carry these three
...
def test_bash_script_checks_bak_present() -> None:
    """After the first install the script must verify ``settings.json.bak.*``.
    ...
    """
    content = _read(BASH_SCRIPT)
    assert re.search(r"settings\.json\.bak", content) or re.search(
        r"\.bak\b", content
...
def test_bash_script_preserves_neighbour_entry_after_uninstall() -> None:
    ...
    """
    content = _read(BASH_SCRIPT)
    assert "crossmem" not in content[:0] or '"crossmem" not in servers' in content, (
...
def test_ps1_script_seeds_fake_home_with_minimal_gemini_config() -> None:
    ...
    The Gemini CLI config path is platform-independent
    (``~/.gemini/settings.json``), so no ``%APPDATA%`` redirect is
    ...
    assert ".gemini" in content and "settings.json" in content, (
        "PowerShell script does not target ~/.gemini/settings.json"
    )
    assert "mcpServers" in content, (
...
def test_ps1_script_validates_command_args_env_keys() -> None:
    ...
    Keeps the Linux + Windows runners' coverage symmetric: a Gemini
...
def test_ps1_script_checks_bak_present() -> None:
    """PS1 script must check the ``settings.json.bak`` glob after first install.
...
def test_scripts_target_the_same_gemini_path() -> None:
    """Both scripts must point at ``~/.gemini/settings.json``.

    Gemini CLI's MCP config path is platform-independent (see
    ``_gemini_path`` in ``crossmem.connectors.registry``). The Windows
    script may spell the separator either way (``/`` or ``\\``) since
    PowerShell accepts both — but both must reference the same final
    component ``settings.json`` under a ``.gemini`` directory.
    """
    bash = _read(BASH_SCRIPT)
    ps1 = _read(PS1_SCRIPT)
    assert ".gemini" in bash and "settings.json" in bash
    assert ".gemini" in ps1 and "settings.json" in ps1
...
def test_scripts_use_the_same_scenario_name() -> None:
    ...
    bash = _read(BASH_SCRIPT)
    ps1 = _read(PS1_SCRIPT)
    assert "scenarios/gemini-cli/install-validate.sh" in bash, (
        "bash report fragment does not embed its canonical scenario name"
    )
    assert "scenarios/gemini-cli/install-validate.ps1" in ps1, (
...
@pytest.mark.parametrize(
    "rel_path",
    [
        "tests/e2e/docker/scenarios/gemini-cli/install-validate.sh",
    ],
)
