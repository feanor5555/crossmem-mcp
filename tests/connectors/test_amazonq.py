"""Tests for the Amazon Q CLI MCP connector."""
...
def test_name() -> None:
    assert AmazonQConnector().name() == "amazonq"


def test_config_path(fake_home: Path) -> None:
    expected = fake_home / ".aws" / "amazonq" / "mcp.json"
    assert AmazonQConnector().config_path() == expected


def test_detect_false_when_missing(fake_home: Path) -> None:
    assert AmazonQConnector().detect() is False


def test_detect_true_when_file_exists(fake_home: Path) -> None:
    cfg = fake_home / ".aws" / "amazonq" / "mcp.json"
    cfg.parent.mkdir(parents=True)
    cfg.write_text("{}", encoding="utf-8")
    assert AmazonQConnector().detect() is True


def test_register_creates_parents_and_entry(fake_home: Path) -> None:
    AmazonQConnector().register("crossmem")
...
def test_unregister_missing_file_is_noop(fake_home: Path) -> None:
    AmazonQConnector().unregister()
    assert not (fake_home / ".aws" / "amazonq" / "mcp.json").exists()


def test_roundtrip(fake_home: Path) -> None:
    connector = AmazonQConnector()
