"""Tests for the Cline (VSCode extension) MCP connector."""
...
def test_name() -> None:
    assert ClineConnector().name() == "cline"
...
def test_config_path_linux(fake_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert ClineConnector().config_path() == expected
...
def test_detect_false_when_missing(
    ...
    monkeypatch.setattr(vscode_module.sys, "platform", "linux")
    assert ClineConnector().detect() is False
...
def test_detect_true_when_file_exists(
    ...
    assert ClineConnector().detect() is True
...
def test_register_creates_nested_parents(
    ...
    ClineConnector().register("crossmem")
...
def test_register_preserves_other_servers_and_backs_up(
    ...
    backups = list(cfg.parent.glob("cline_mcp_settings.json.bak.*"))
...
def test_config_path_env_override_wins_over_platform_default(
    ...
    # Platform default would point elsewhere; env-override must win
